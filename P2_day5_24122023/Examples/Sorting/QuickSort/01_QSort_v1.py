...
def qsort_inplace(lst, start=0, end=None):
    ...
    if end is None:
        end = len(lst) - 1  # Для нашего списка end = 14
    if end - start < 1:
        return None

    pivot_index = random.randint(start, end) # pivot_index берется случайным образом

    print(f'pivot_index = {pivot_index}, value = {lst[pivot_index]}')
    print('lst:', lst)

    x = subpart(lst, start, end, pivot_index) # запуск вложенной функции
